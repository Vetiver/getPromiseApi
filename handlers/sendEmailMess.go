...
func (h *BaseHandler) sendConfirmationEmail(reqData *db.User, code int) (string) {
	...
	m.SetBody("text/html", fmt.Sprintf("Спасибо за регистрацию, вот ваша ссылка на подтверждение: <a href=\"http://localhost:8000/register?code=%d\">http://localhost:8000/register?code=%d</a>", jwtCode, jwtCode))
